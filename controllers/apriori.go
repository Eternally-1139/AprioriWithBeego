...
//@router /
func (this *AprioriController) Index(){
	var strs string

	var bank []*models.BankData
	orm.NewOrm().QueryTable("bank_data").All(&bank)
	struser := "uId  IsMarried  Car  SaveAct "+"\n"
	tracefile(struser)
	for _, bank_data := range bank {
		str:= bank_data.Id+"  "+bank_data.Married+"  "+bank_data.Car+ "  "+bank_data.SaveAct+"  "
		fmt.Printf(str)
		tracefile(str)
	}
	...
	apriori.record ,_= apriori.getRecord("apriori.txt")
	...
	for !apriori.endTag {
		...
		if len(apriori.confItemSet) != 0 { // 满足置信度的集合不为空
			for i := 0; i < len(apriori.confItemSet); i++ {
				var ret string
				j := 0
				for j = 0; j < len(apriori.confItemSet[i])-3; j++ {
					ret += apriori.confItemSet[i][j] + " "
				}

				ret += "---> "
				ret += apriori.confItemSet[i][j] + " "
				j++
				ret += "支持度: " + apriori.confItemSet[i][j]
				j++
				ret += ", 置信度: " + apriori.confItemSet[i][j]
				j++
				log.Println(ret)
				strs+=ret
			}
...
func (this *Apriori) result(confItemset2 [][]string) {
	...
	for i := 0; i < len(confItemset2); i++ {
		...
		ret += confItemset2[i][j] + " "
		j++
		ret += "支持度: " + confItemset2[i][j]
		j++
		ret += ", 置信度: " + confItemset2[i][j]
		j++
		log.Println(ret)
	}
...
func (this *Apriori) getRecord(filename string) ([][]string, error) {
	...
	for {
		...
		for i := 0; i < len(lineStr); i++ {
			if strings.HasSuffix(lineStr[i], "YES") ||
				strings.HasSuffix(lineStr[i], "YES") {
				lineList = append(lineList, record[0][i])
			} else if strings.HasSuffix(lineStr[i], "NO") ||
...
func tracefile(str_content string)  {
	fd,_:=os.OpenFile("apriori.txt",os.O_RDWR|os.O_CREATE|os.O_APPEND,0644)
